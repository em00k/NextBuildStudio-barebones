...
class CSpectImageWrapper:
    def __init__(self, target_dir="./img/", force=False):
...
def main():
    ...
    print(f"{YELLOW}NextBuild Studio CSpect Next 2GB Image Wrapper - em00k 04.07.2025 - v1.0{RESET}")

    print(f"{GREEN}✓ Using built-in Python libraries (no external dependencies required){RESET}")
    print()

    # Run the wrapper
    wrapper = CSpectImageWrapper(target_dir=args.target_dir, force=args.force)
    return wrapper.run()
