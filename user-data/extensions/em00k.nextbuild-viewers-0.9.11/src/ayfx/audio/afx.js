...
const maxEffectFrames = 0xFFF; // 4095 frames max per effect
const maxAFBSize = 0x1FFF; // 8191 bytes max AFB size
...
export class Effect {
  ...
  export() {
    ...
    const length = Math.min(this.length, maxEffectFrames); // Silently limit to max frames
    let tone = 0;
    let noise = 0;
    let ptr = 0;
    let it;
  ...
  load(data, length = data.length) {
    ...
    for (; offset < length; ) {
      ...
      if (byte & (1 << 5)) {
        currentTone = view.getUint16(offset, true) & 0xfff;
        offset += 2;
      }

      if (byte & (1 << 6)) {
        currentNoise = view.getUint8(offset++);
        if (byte === 0xd0 && currentNoise >= 0x20) {
          break;
        }
        currentNoise &= 0x1f;
      }

      frame.tone = currentTone;
      frame.noise = currentNoise;
...
export class Bank extends Hooks {
  ...
  export() {
    ...
    for (i = 0; i < length; i++) {
      ...
      // Check if adding this effect would exceed AFB size limit
      if (pp + effectData.length + (effect.name.length ? effect.name.length + 1 : 0) > maxAFBSize) {
        // Silently truncate - stop adding more effects
        break;
      }
